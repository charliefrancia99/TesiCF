...
leftHipX = data.LEFT_HIPX;
leftHipY = data.LEFT_HIPY;
leftHipZ = data.LEFT_HIPZ;

leftKneeX = data.LEFT_KNEEX;
leftKneeY = data.LEFT_KNEEY;
leftKneeZ = data.LEFT_KNEEZ;

leftAnkleX = data.LEFT_ANKLEX;
leftAnkleY = data.LEFT_ANKLEY;
leftAnkleZ = data.LEFT_ANKLEZ;

% Calcola l'angolo destro tra hip-knee e knee-ankle a livello del ginocchio
numFrames = length(leftHipX);

angoloSinistroGinocchio = zeros(numFrames, 1);

for i = 1:numFrames
    hipKneeVector = [leftKneeX(i) - leftHipX(i), leftKneeY(i) - leftHipY(i), leftKneeZ(i) - leftHipZ(i)];
    kneeAnkleVector = [leftAnkleX(i) - leftKneeX(i), leftAnkleY(i) - leftKneeY(i), leftAnkleZ(i) - leftKneeZ(i)];
...
title('Angolo Sinistro ginocchio');
xlabel('Time (s)');
ylabel('Angolo (gradi)');
grid on;
...
frame = xls_data(:, 1);
time2=frame/60;
left_knee_flexion_extension = xls_data(:, 61);

% Crea il grafico
figure;
plot(time2, left_knee_flexion_extension, 'b.-');
xlabel('Time (s)');
ylabel('Left Knee Flexion/Extension');
title('Grafico Left Hip Flexion/Extension vs. Frame');
grid on;

% Opzionale: Aggiungi una legenda o ulteriori personalizzazioni al grafico
...
if lunghezza_diff < 0
    ...
    left_knee_flexion_extension = left_knee_flexion_extension(-lunghezza_diff : end-1);
    time2 = time2(1:length(left_knee_flexion_extension)); % Aggiorna il secondo vettore tempo
elseif lunghezza_diff > 0
    % Rimuovi gli elementi iniziali in eccesso dal primo angolo
    angoloSinistroGinocchioFiltrato = angoloSinistroGinocchioFiltrato(lunghezza_diff : end-1);
    time1 = time1(1:length(angoloSinistroGinocchioFiltrato)); % Aggiorna il primo vettore tempo
end
...
subplot(2, 1, 1);
plot(time1, angoloSinistroGinocchioFiltrato, 'b-', 'LineWidth', 2);
xlabel('Time(s)');
...
%subplot(2, 1, 2);
plot(time2, left_knee_flexion_extension, 'r-', 'LineWidth', 2);
xlabel('Time(s)');
...
%% SINCRONIZZAZIONE 2 se piu lungo left knee
...
subplot(2, 1, 2);
plot(time2, left_knee_flexion_extension, 'b.-');
title('Left Knee Flexion/Extension');
xlabel('Time (s)');
ylabel('Left Knee Flexion/Extension');
grid on;

% Scegli manualmente un punto su angoloDestroGinocchioFiltrato
disp('Seleziona un punto su angoloSinistroGinocchioFiltrato.');
[x1, ~] = ginput(1);

% Scegli manualmente un punto su right_knee_flexion_extension
disp('Seleziona un punto su left_knee_flexion_extension.');
[x2, ~] = ginput(1);
...
angoloSinistroGinocchioFiltratoSync = interp1(time1_sync, angoloSinistroGinocchioFiltrato, time1, 'linear', 'extrap');
angoloSinistroGinocchioFiltrato=angoloSinistroGinocchioFiltratoSync;
left_knee_flexion_extension=left_knee_flexion_extension(1:length(angoloSinistroGinocchioFiltrato));
% Aggiorna il grafico sincronizzato sovrapposto
figure
plot(time1, angoloSinistroGinocchioFiltrato, 'r-', 'LineWidth', 2);
hold on;
plot(time1, left_knee_flexion_extension, 'b.-');
hold off;

title('Grafici Sovrapposti - Sincronizzati');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
legend('Angolo sinistro ginocchio (Filtrato)', 'Left Knee Flexion/Extension');
grid on;
time2=time1;
...
plot(time1, angoloSinistroGinocchioFiltrato, 'Color', MATLABcol(1,:), 'LineWidth', 2);
hold on;

% Plot della flessione/estensione continua del ginocchio destro (Xsens)
plot(time2, left_knee_flexion_extension, 'Color', MATLABcol(2,:), 'LineWidth', 2);

hold off;

title('Left Knee Flexion/Extension Comparison','interpreter', 'latex');
...
xlim([0;11]); % Imposta il limite inferiore dell'asse x a 10 secondi
xticks(0:2:11);
ylim([0;140]);
...
[h1, p1, jbstat1, critval1] = jbtest(angoloSinistroGinocchioFiltrato);

% Stampare i risultati del test per angoloDestroGinocchioFiltrato
fprintf('Risultati del test di Jarque-Bera per angoloSInistroGinocchioFiltrato:\n');
...
[h2, p2, jbstat2, critval2] = jbtest(left_knee_flexion_extension);

% Stampare i risultati del test per right_knee_flexion_extension
fprintf('\nRisultati del test di Jarque-Bera per left_knee_flexion_extension:\n');
fprintf('Statistiche JB: %.4f\n', jbstat2);
fprintf('Valore p: %.4f\n', p2);
if h2
    fprintf('I dati NON seguono una distribuzione normale.\n');
...
rho = corr(angoloSinistroGinocchioFiltrato, left_knee_flexion_extension, 'Type', 'Spearman');
fprintf('Il coefficiente di correlazione di Spearman tra i due segnali è: %.4f\n', rho);

%% ICC NUOVO
% Calcola l'ICC
[r, LB, UB, F, df1, df2, p] = ICC([angoloSinistroGinocchioFiltrato, left_knee_flexion_extension], 'A-1', 0.05, 0);

% Stampa i risultati
fprintf('ICC tra angoloSinistroGinocchioFiltrato e left_knee_flexion_extension:\n');
...
% Calcolo del bias
bias = mean(left_knee_flexion_extension - angoloSinistroGinocchioFiltrato);
fprintf('Il bias tra i due segnali è: %.4f\n', bias);

% Calcolo della deviazione standard
std_dev = std(left_knee_flexion_extension - angoloSinistroGinocchioFiltrato);
fprintf('La deviazione standard per la differenza dei  due segnali è: %.4f\n', std_dev);

%CV
% Calcolo del Coefficiente di Variazione (CV)
cv = (std_dev / mean((angoloSinistroGinocchioFiltrato + left_knee_flexion_extension)/2)) * 100;
...
% Bland-Altman plot
BlandAltman(angoloSinistroGinocchioFiltrato, left_knee_flexion_extension, {'Angolo destro ginocchio', 'Angolo ginocchio'});
title('Bland-Altman Plot tra Angolo sinistro ginocchio e Angolo ginocchio');
...
differenza = left_knee_flexion_extension - angoloSinistroGinocchioFiltrato;
media = mean([angoloSinistroGinocchioFiltrato, left_knee_flexion_extension], 2);
...
offset_x = 5; % Modifica questo valore per regolare lo spostamento orizzontale
offset_y = 8; % Modifica questo valore per regolare lo spostamento verticale
...
title('Bland-Altman Plot Left Knee Flexion/Extension', 'interpreter', 'latex');
...
kinect_segment = angoloSinistroGinocchioFiltrato(1:i);

opto_segment = left_knee_flexion_extension(1:i);
...
rom_angoloSinistro = max(angoloSinistroGinocchioFiltrato) - min(angoloSinistroGinocchioFiltrato);
fprintf('Range of Motion per angoloSinistroGinocchioFiltrato: %.2f gradi\n', rom_angoloSinistro);

%% ROM XSENS 
% Calcola il ROM per "right_knee_flexion_extension"
rom_left_knee = max(left_knee_flexion_extension) - min(left_knee_flexion_extension);

fprintf('Range of Motion per left_knee_flexion_extension: %.2f gradi\n', rom_left_knee);
...
for window = 1:4
    ...
    angolo_window = angoloSinistroGinocchioFiltrato(window_start_index:window_end_index);
    
    % Seleziona i dati ottici all'interno della finestra corrente
    opto_window = left_knee_flexion_extension(window_start_index:window_end_index);
    ...
    if window >= 2
        angolo_finestre_selezionate = angoloSinistroGinocchioFiltrato(window_start_index:window_end_index);
        opto_finestre_selezionate = left_knee_flexion_extension(window_start_index:window_end_index);
...
fprintf('ROM per angoloSinistroGinocchioFiltrato nelle finestre selezionate:\n');
for window = 1:4
    fprintf('Finestra %d: %.2f gradi\n', window, rom_values(window, 1));
end

fprintf('ROM per left_knee_flexion_extension nelle finestre selezionate:\n');
for window = 1:4
    fprintf('Finestra %d: %.2f gradi\n', window, rom_values(window, 2));
end

% Visualizza le deviazioni standard
fprintf('Deviazione standard per angoloSinistroGinocchioFiltrato nelle finestre 2, 3 e 4: %.2f gradi\n', mean(dev_std_angolo));
fprintf('Deviazione standard per left_knee_flexion_extension nelle finestre 2, 3 e 4: %.2f gradi\n', mean(dev_std_opto));
