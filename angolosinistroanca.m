...
sinistroPtoBX = data.LEFT_HIPX;
sinistroPtoBY = data.LEFT_HIPY+20;
sinistroPtoBZ = data.LEFT_HIPZ;

% Calcola l'angolo sinistro livello dell'anca
...
for i = 1:numFrames
    ...
    hipptoBVector = [sinistroPtoBX(i) - sinistroHipX(i), sinistroPtoBY(i) - sinistroHipY(i), sinistroPtoBZ(i) - sinistroHipZ(i)];

    dotProduct = dot(hipptoBVector,hipKneeVector);
    normHipKnee = norm(hipKneeVector);
    normHipptoB = norm(hipptoBVector);

    angoloSinistroAnca(i) = acosd(dotProduct / (normHipKnee * normHipptoB));
end
angoloSinistroAnca=angoloSinistroAnca';

% Plot dell'angolo rispetto ai frame
frames = data.Frame;
time1= frames/60;
figure
...
frame = xls_data(:, 1);
time2=frame/60;
left_hip_flexion_extension = xls_data(:, 58);

% Crea il grafico
figure;
plot(time2, left_hip_flexion_extension, 'b.-');
xlabel('Time (s)');
ylabel('Left Hip Flexion/Extension');
title('Grafico Hip Knee Flexion/Extension vs. Time');
grid on;
...
if lunghezza_diff < 0
    ...
    left_hip_flexion_extension = left_hip_flexion_extension(-lunghezza_diff : end-1);
    time2 = time2(1:length(left_hip_flexion_extension)); % Aggiorna il secondo vettore tempo
elseif lunghezza_diff > 0
    % Rimuovi gli elementi iniziali in eccesso dal primo angolo
    angoloSinistroAncaFiltrato = angoloSinistroAncaFiltrato(lunghezza_diff : end-1);
    time1 = time1(1:length(angoloSinistroAncaFiltrato)); % Aggiorna il primo vettore tempo
end
...
subplot(2, 1, 1);
plot(time1, angoloSinistroAncaFiltrato, 'b-', 'LineWidth', 2);
xlabel('Time(s)');
ylabel('Angolo sinistro anca (gradi)');
title('Angolo sinistro anca vs. Time');
grid on;

hold on
% Sottoplot per il secondo angolo
%subplot(2, 1, 2);
plot(time2, left_hip_flexion_extension, 'r-', 'LineWidth', 2);
xlabel('Time(s)');
ylabel('Angolo anca (gradi)');
title('Angolo anca vs. Time');
grid on;
i=length(time1);
...
subplot(2, 1, 2);
plot(time2, left_hip_flexion_extension, 'b.-');
title('Left Hip Flexion/Extension');
xlabel('Time (s)');
ylabel('Left hip Flexion/Extension');
grid on;

% Scegli manualmente un punto su angoloSinistroAncaFiltrato
disp('Seleziona un punto su angoloSinistroAncaFiltrato.');
[x1, ~] = ginput(1);

% Scegli manualmente un punto su left_hip_flexion_extension
...
% Interpola per ottenere la sincronizzazione
time1_sync = time1 + temporalDifference;
angoloSinistroAncaFiltratoSync = interp1(time1_sync, angoloSinistroAncaFiltrato, time1, 'linear', 'extrap');
angoloSinistroAncaFiltrato=angoloSinistroAncaFiltratoSync;
left_hip_flexion_extension=left_hip_flexion_extension(1:length(angoloSinistroAncaFiltrato));
% Aggiorna il grafico sincronizzato sovrapposto
figure
plot(time1, angoloSinistroAncaFiltrato, 'r-', 'LineWidth', 2);
hold on;
plot(time1, left_hip_flexion_extension, 'b.-');
hold off;

title('Grafici Sovrapposti - Sincronizzati');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
legend('Angolo sinistro anca (Filtrato)', 'Left hip Flexion/Extension');
grid on;
time2=time1;
...
% Plot della flessione/estensione continua del anca sinistra (Mediapipe)
plot(time1, angoloSinistroAncaFiltrato, 'Color', MATLABcol(1,:), 'LineWidth', 2);
hold on;

% Plot della flessione/estensione continua del anca sinistra (Xsens)
plot(time2, left_hip_flexion_extension, 'Color', MATLABcol(2,:), 'LineWidth', 2);

hold off;

title('Left Hip Flexion/Extension Comparison','interpreter', 'latex');
...
xlim([0;10]); % Imposta il limite inferiore dell'asse x a 10 secondi
xticks(0:2:12);
set(gca,'TickLabelInterpreter','latex');
grid on
%%
exportgraphics(figName, "grafico_left_knee.pdf");

%% TEST di Jarque-Bera
% Esegui il test di Jarque-Bera 1
[h1, p1, jbstat1, critval1] = jbtest(angoloSinistroAncaFiltrato);

% Stampare i risultati del test 
fprintf('Risultati del test di Jarque-Bera per angoloSinistroAncaFiltrato:\n');
...
% Esegui il test di Jarque-Bera 2
[h2, p2, jbstat2, critval2] = jbtest(left_hip_flexion_extension);

% Stampare i risultati del test 
fprintf('\nRisultati del test di Jarque-Bera per leftt_hip_flexion_extension:\n');
fprintf('Statistiche JB: %.4f\n', jbstat2);
fprintf('Valore p: %.4f\n', p2);
if h2
    fprintf('I dati NON seguono una distribuzione normale.\n');
...
rho = corr(angoloSinistroAncaFiltrato, left_hip_flexion_extension, 'Type', 'Spearman');
fprintf('Il coefficiente di correlazione di Spearman tra i due segnali è: %.4f\n', rho);

%% ICC NUOVO
% Calcola l'ICC
[r, LB, UB, F, df1, df2, p] = ICC([angoloSinistroAncaFiltrato, left_hip_flexion_extension], 'A-1', 0.05, 0);

% Stampa i risultati
fprintf('ICC tra angoloSinistroAncaFiltrato e left_hip_flexion_extension:\n');
...
% Calcolo del bias
bias = mean(left_hip_flexion_extension - angoloSinistroAncaFiltrato);
fprintf('Il bias tra i due segnali è: %.4f\n', bias);

% Calcolo della deviazione standard
std_dev = std(left_hip_flexion_extension - angoloSinistroAncaFiltrato);
fprintf('La deviazione standard per la differenza dei  due segnali è: %.4f\n', std_dev);

%CV
% Calcolo del Coefficiente di Variazione (CV)
cv = (std_dev / mean((angoloSinistroAncaFiltrato + left_hip_flexion_extension)/2)) * 100;
...
% Bland-Altman plot
BlandAltman(angoloSinistroAncaFiltrato, left_hip_flexion_extension, {'Angolo Sinistro Anca 1', 'Angolo Sinistro Anca 2'});
title('Bland-Altman Plot tra Angoli Sinistro anca');
...
differenza = left_hip_flexion_extension - angoloSinistroAncaFiltrato;
media = mean([angoloSinistroAncaFiltrato, left_hip_flexion_extension], 2);
...
% Definisci gli offset per spostare l'etichetta
offset_x = 2; % Modifica questo valore per regolare lo spostamento orizzontale
offset_y = -1; % Modifica questo valore per regolare lo spostamento verticale

text(min(media) + offset_x, max(differenza) + offset_y, sd_label, ...
...
title('Bland-Altman Plot Left Hip Flexion/Extension', 'interpreter', 'latex');
...
%%
exportgraphics(figName, "baplotLKFE_right_knee.pdf");


%% RMSE giusto

kinect_segment = angoloSinistroAncaFiltrato(1:i);

opto_segment = left_hip_flexion_extension(1:i);
...
% Calcola il ROM per angoloSinistroAncaFiltrato
rom_angoloSinistro = max(angoloSinistroAncaFiltrato) - min(angoloSinistroAncaFiltrato);

fprintf('Range of Motion per angoloSinistroAncaFiltrato: %.2f gradi\n', rom_angoloSinistro);

%% ROM XSENS
% Calcola il ROM per Left_hip_flexion_extension
rom_left_hip = max(left_hip_flexion_extension) - min(left_hip_flexion_extension);

fprintf('Range of Motion per left_hip_flexion_extension: %.2f gradi\n', rom_left_hip);
...
for window = 1:4
    ...
    angolo_window = angoloSinistroAncaFiltrato(window_start_index:window_end_index);
    
    % Seleziona i dati ottici all'interno della finestra corrente
    opto_window = left_hip_flexion_extension(window_start_index:window_end_index);
    ...
    if window >= 2
        angolo_finestre_selezionate = angoloSinistroAncaFiltrato(window_start_index:window_end_index);
        opto_finestre_selezionate = left_hip_flexion_extension(window_start_index:window_end_index);
...
fprintf('ROM per angoloSinistroAncaFiltrato nelle finestre selezionate:\n');
for window = 1:4
    fprintf('Finestra %d: %.2f gradi\n', window, rom_values(window, 1));
end

fprintf('ROM per left_hip_flexion_extension nelle finestre selezionate:\n');
for window = 1:4
