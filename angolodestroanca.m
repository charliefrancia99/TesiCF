...
destroPtoBX = data.RIGHT_HIPX;
destroPtoBY = data.RIGHT_HIPY+20;
destroPtoBZ = data.RIGHT_HIPZ;

% Calcola l'angolo destro livello dell'anca
...
for i = 1:numFrames
    ...
    hipptoBVector = [destroPtoBX(i) - destroHipX(i), destroPtoBY(i) - destroHipY(i), destroPtoBZ(i) - destroHipZ(i)];

    dotProduct = dot(hipptoBVector,hipKneeVector);
    normHipKnee = norm(hipKneeVector);
    normHipptoB = norm(hipptoBVector);

    angoloDestroAnca(i) = acosd(dotProduct / (normHipKnee * normHipptoB));
end
angoloDestroAnca=angoloDestroAnca';
...
% Filtra i dati dell'angolo destro anca
...
frame = xls_data(:, 1);
time2=frame/60;
right_hip_flexion_extension = xls_data(:, 46);

% Crea il grafico
figure;
plot(time2, right_hip_flexion_extension, 'b.-');
xlabel('Time (s)');
ylabel('Right Hip Flexion/Extension');
title('Grafico Right Hip Flexion/Extension vs. Time');
grid on;
...
if lunghezza_diff < 0
    ...
    right_hip_flexion_extension = right_hip_flexion_extension(-lunghezza_diff : end-1);
    time2 = time2(1:length(right_hip_flexion_extension)); % Aggiorna il secondo vettore tempo
elseif lunghezza_diff > 0
    % Rimuovi gli elementi iniziali in eccesso dal primo angolo
    angoloDestroAncaFiltrato = angoloDestroAncaFiltrato(lunghezza_diff : end-1);
    time1 = time1(1:length(angoloDestroAncaFiltrato)); % Aggiorna il primo vettore tempo
end
...
subplot(2, 1, 1);
plot(time1, angoloDestroAncaFiltrato, 'b-', 'LineWidth', 2);
xlabel('Time(s)');
ylabel('Angolo Destro anca (gradi)');
title('Angolo Destro anca vs. Time');
grid on;

hold on
% Sottoplot per il secondo angolo
%subplot(2, 1, 2);
plot(time2, right_hip_flexion_extension, 'r-', 'LineWidth', 2);
xlabel('Time(s)');
...
% Scegli manualmente un punto 
disp('Seleziona un punto su angoloDestroAncaFiltrato.');
[x1, ~] = ginput(1);

% Scegli manualmente un punto 
disp('Seleziona un punto su right_hip_flexion_extension.');
[x2, ~] = ginput(1);
...
angoloDestroAncaFiltratoSync = interp1(time1_sync, angoloDestroAncaFiltrato, time1, 'linear', 'extrap');
angoloDestroAncaFiltrato=angoloDestroAncaFiltratoSync;
right_hip_flexion_extension=right_hip_flexion_extension(1:length(angoloDestroAncaFiltrato));
% Aggiorna il grafico sincronizzato sovrapposto
figure
plot(time1, angoloDestroAncaFiltrato, 'r-', 'LineWidth', 2);
hold on;
plot(time1, right_hip_flexion_extension, 'b.-');
hold off;

title('Grafici Sovrapposti - Sincronizzati');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
legend('Angolo destro anca (Filtrato)', 'Right hip Flexion/Extension');
grid on;
time2=time1;
...
% Plot della flessione/estensione continua del anca destro (Mediapipe)
plot(time1, angoloDestroAncaFiltrato, 'Color', MATLABcol(1,:), 'LineWidth', 2);
hold on;

% Plot della flessione/estensione continua del ancadestro (Xsens)
plot(time2, right_hip_flexion_extension, 'Color', MATLABcol(2,:), 'LineWidth', 2);

hold off;

title('Right Hip Flexion/Extension Comparison','interpreter', 'latex');
...
xlim([14;24]); % Imposta il limite inferiore dell'asse x a 10 secondi
ylim([-20;140]);
xticks(10:2:max(time1));
...
[h1, p1, jbstat1, critval1] = jbtest(angoloDestroAncaFiltrato);

% Stampare i risultati del test 
fprintf('Risultati del test di Jarque-Bera per angoloDestroAncaFiltrato:\n');
...
[h2, p2, jbstat2, critval2] = jbtest(right_hip_flexion_extension);

% Stampare i risultati del test 
fprintf('\nRisultati del test di Jarque-Bera per right_hip_flexion_extension:\n');
fprintf('Statistiche JB: %.4f\n', jbstat2);
fprintf('Valore p: %.4f\n', p2);
if h2
    fprintf('I dati NON seguono una distribuzione normale.\n');
...
rho = corr(angoloDestroAncaFiltrato, right_hip_flexion_extension, 'Type', 'Spearman');
...
% Stampa i risultati
fprintf('ICC tra angoloDestroAncaFiltrato e right_hip_flexion_extension:\n');
...
% Calcolo del bias
bias = mean(right_hip_flexion_extension - angoloDestroAncaFiltrato);
fprintf('Il bias tra i due segnali è: %.4f\n', bias);

% Calcolo della deviazione standard
std_dev = std(right_hip_flexion_extension - angoloDestroAncaFiltrato);
fprintf('La deviazione standard per la differenza dei  due segnali è: %.4f\n', std_dev);

%CV
% Calcolo del Coefficiente di Variazione (CV)
cv = (std_dev / mean((angoloDestroAncaFiltrato + right_hip_flexion_extension)/2)) * 100;

fprintf('Il Coefficiente di Variazione tra i due segnali è: %.4f%%\n', cv);

% Bland-Altman plot
BlandAltman(angoloDestroAncaFiltrato, right_hip_flexion_extension, {'Angolo destro ginocchio', 'Angolo ginocchio'});
...
differenza = right_hip_flexion_extension - angoloDestroAncaFiltrato;
media = mean([angoloDestroAncaFiltrato, right_hip_flexion_extension], 2);
...
offset_y = 11; % Modifica questo valore per regolare lo spostamento verticale
...
xlim([0, 120]);
ylim([-25, 25]);
title('Bland-Altman Plot Right Hip Flexion/Extension', 'interpreter', 'latex');
...
kinect_segment = angoloDestroAncaFiltrato(1:i);

opto_segment = right_hip_flexion_extension(1:i);
...
%% ROM GOPRO
% Calcola il ROM
rom_angoloDestro = max(angoloDestroAncaFiltrato) - min(angoloDestroAncaFiltrato);
fprintf('Range of Motion per angoloDestroAncaFiltrato: %.2f gradi\n', rom_angoloDestro);


%% ROM XSENS
% Calcola il ROM 
rom_right_hip = max(right_hip_flexion_extension) - min(right_hip_flexion_extension);

fprintf('Range of Motion per right_hip_flexion_extension: %.2f gradi\n', rom_right_hip);

%% RMSE E ROM CALCOLATO NELLE DIVERSE FINESTRE TEMPORALI

% Disegna il grafico dell'angolo destro
figure;
plot(time1, angoloDestroAncaFiltrato, 'b-', 'LineWidth', 2);
hold on;
plot(time2, right_hip_flexion_extension, 'r-', 'LineWidth', 2);
...
for window = 1:4
    ...
    angolo_window = angoloDestroAncaFiltrato(window_start_index:window_end_index);
    
    % Seleziona i dati ottici all'interno della finestra corrente
    opto_window = right_hip_flexion_extension(window_start_index:window_end_index);
    ...
    if window >= 2
        angolo_finestre_selezionate = angoloDestroAncaFiltrato(window_start_index:window_end_index);
        opto_finestre_selezionate = right_hip_flexion_extension(window_start_index:window_end_index);
...
fprintf('ROM per angoloDestroAncaFiltrato nelle finestre selezionate:\n');
for window = 1:4
    fprintf('Finestra %d: %.2f gradi\n', window, rom_values(window, 1));
end

fprintf('ROM per right_hip_flexion_extension nelle finestre selezionate:\n');
for window = 1:4
    fprintf('Finestra %d: %.2f gradi\n', window, rom_values(window, 2));
end

% Visualizza le deviazioni standard
fprintf('Deviazione standard per angoloDestroAncaFiltrato nelle finestre 2, 3 e 4: %.2f gradi\n', mean(dev_std_angolo));
fprintf('Deviazione standard per right_hip_flexion_extension nelle finestre 2, 3 e 4: %.2f gradi\n', mean(dev_std_opto));
