...
angoloSpallaDestro = zeros(numFrames, 1);
...
for i = 1:numFrames
    ...
    angoloSpallaDestro(i) = acosd(dotProduct / (normShoulderCustomPoint * normShoulderElbow));
end
...
time1 = frames / 60;
figure
plot(time1, angoloSpallaDestro, 'b-', 'LineWidth', 2);

% Personalizza il grafico
title('Angolo di abduzione-adduzione della spalla destra');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
grid on;
...
angoloSpallaDestroFiltrato = filtfilt(b, a, angoloSpallaDestro);

% Plot dell'angolo destro filtrato rispetto ai frame
figure
plot(time1, angoloSpallaDestroFiltrato, 'r-', 'LineWidth', 2);

% Personalizza il grafico
title('Angolo destro spalla (Filtrato)');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
grid on;
...
frame = xls_data(:, 1);
time2=frame/60;
right_shoulder_abduction_adduction = xls_data(:, 23);

% Crea il grafico
figure;
plot(time2, right_shoulder_abduction_adduction, 'b.-');
xlabel('Time (s)');
ylabel('Right shoulder abduction adduction');
title('Grafico Right shoulder abduction adduction vs. Time');
grid on;
%% SINCRONIZZAZIONE
...
if lunghezza_diff < 0
    % Rimuovi gli elementi iniziali in eccesso dal secondo angolo
     right_shoulder_abduction_adduction = right_shoulder_abduction_adduction(-lunghezza_diff : end-1);
    time2 = time2(1:length(right_shoulder_abduction_adduction)); % Aggiorna il secondo vettore tempo
elseif lunghezza_diff > 0
    % Rimuovi gli elementi iniziali in eccesso dal primo angolo
    angoloSpallaDestroFiltrato = angoloSpallaDestroFiltrato(lunghezza_diff : end-1);
    time1 = time1(1:length(angoloSpallaDestroFiltrato)); % Aggiorna il primo vettore tempo
end
...
subplot(2, 1, 1);
plot(time1, angoloSpallaDestroFiltrato, 'b-', 'LineWidth', 2);
xlabel('Time(s)');
ylabel('Angolo destro spalla (gradi)');
title('Angolo destro spalla vs. Time');
grid on;

hold on
% Sottoplot per il secondo angolo
%subplot(2, 1, 2);
plot(time2, right_shoulder_abduction_adduction, 'r-', 'LineWidth', 2);
xlabel('Time(s)');
ylabel('Angolo destro spalla (gradi)');
title('Angolo destro spalla vs. Time');
grid on;
...
subplot(2, 1, 1);
plot(time1, angoloSpallaDestroFiltrato, 'b-', 'LineWidth', 2);
title('Angolo angoloSpallaDestroFiltrato');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
grid on;
...
subplot(2, 1, 2);
plot(time2, right_shoulder_abduction_adduction, 'b.-');
title('right_shoulder_abduction_adduction');
xlabel('Time (s)');
ylabel('right_shoulder_abduction_adduction');
grid on;

% Scegli manualmente un punto su angoloDestroGinocchioFiltrato
disp('Seleziona un punto su angoloSpallaDestroFiltrato');
[x1, ~] = ginput(1);

% Scegli manualmente un punto su right_knee_flexion_extension
disp('Seleziona un punto su right_shoulder_abduction_adduction.');
[x2, ~] = ginput(1);
...
angoloSpallaDestroFiltratoSync = interp1(time1_sync, angoloSpallaDestroFiltrato, time1, 'linear', 'extrap');
angoloSpallaDestroFiltrato=angoloSpallaDestroFiltratoSync;
right_shoulder_abduction_adduction=right_shoulder_abduction_adduction(1:length(angoloSpallaDestroFiltrato));
% Aggiorna il grafico sincronizzato sovrapposto
figure
plot(time1, angoloSpallaDestroFiltrato, 'r-', 'LineWidth', 2);
hold on;
plot(time1, right_shoulder_abduction_adduction, 'b.-');
hold off;

title('Grafici Sovrapposti - Sincronizzati');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
legend('angoloSpallaDestro(Filtrato)', 'Right Shoulder abduction/adduction');
grid on;
time2=time1;
...
plot(time1, angoloSpallaDestroFiltrato, 'Color', MATLABcol(1,:), 'LineWidth', 2);
hold on;

% Plot della flessione/estensione continua del ginocchio destro (Xsens)
plot(time2, right_shoulder_abduction_adduction, 'Color', MATLABcol(2,:), 'LineWidth', 2);

hold off;

title('Right Shoulder Abduction/Adduction Comparison','interpreter', 'latex');
...
[h1, p1, jbstat1, critval1] = jbtest(angoloSpallaDestroFiltrato);

% Stampare i risultati del test per angoloDestroGinocchioFiltrato
fprintf('Risultati del test di Jarque-Bera per aangoloSpallaDestroFiltrato:\n');
...
[h2, p2, jbstat2, critval2] = jbtest(right_shoulder_abduction_adduction);

% Stampare i risultati del test per right_knee_flexion_extension
fprintf('\nRisultati del test di Jarque-Bera per right_shoulder_abduction_adduction:\n');
fprintf('Statistiche JB: %.4f\n', jbstat2);
fprintf('Valore p: %.4f\n', p2);
if h2
    fprintf('I dati NON seguono una distribuzione normale.\n');
...
rho = corr(angoloSpallaDestroFiltrato, right_shoulder_abduction_adduction, 'Type', 'Spearman');
fprintf('Il coefficiente di correlazione di Spearman tra i due segnali è: %.4f\n', rho);

%% ICC NUOVO
% Calcola l'ICC
[r, LB, UB, F, df1, df2, p] = ICC_copia([angoloSpallaDestroFiltrato, right_shoulder_abduction_adduction], 'A-1', 0.05, 0);

% Stampa i risultati
fprintf('ICC tra angoloSpallaDestroFiltrato e right_shoulder_abduction_adduction:\n');
...
% Calcolo del bias
bias = mean(right_shoulder_abduction_adduction - angoloSpallaDestroFiltrato);
fprintf('Il bias tra i due segnali è: %.4f\n', bias);

% Calcolo della deviazione standard
std_dev = std(right_shoulder_abduction_adduction - angoloSpallaDestroFiltrato);
fprintf('La deviazione standard per la differenza dei  due segnali è: %.4f\n', std_dev);

%CV
% Calcolo del Coefficiente di Variazione (CV)
cv = (std_dev / mean((angoloSpallaDestroFiltrato + right_shoulder_abduction_adduction)/2)) * 100;
...
% Bland-Altman plot
BlandAltman(angoloSpallaDestroFiltrato, right_shoulder_abduction_adduction, {'angoloSpallaDestroFiltrato', 'Angolo spalla'});
title('Bland-Altman Plot tra Angolo destro spalla e Angolo spalla');
...
differenza = right_shoulder_abduction_adduction - angoloSpallaDestroFiltrato;
media = mean([angoloSpallaDestroFiltrato, right_shoulder_abduction_adduction], 2);
...
text(min(media)+2, max(differenza)+4, sd_label, 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left', 'Color', 'black', 'interpreter', 'latex' );
...
title('Bland-Altman Plot Right Shoulder Abduction/Adduction', 'interpreter', 'latex');
...
kinect_segment = angoloSpallaDestroFiltrato(1:i);

opto_segment = right_shoulder_abduction_adduction(1:i);
...
rom_angoloDestro = max(angoloSpallaDestroFiltrato) - min(angoloSpallaDestroFiltrato);
fprintf('Range of Motion per angoloSpallaDestroFiltrato: %.2f gradi\n', rom_angoloDestro);


%% ROM XSENS

% Calcola il ROM per "right_knee_flexion_extension"
rom_right_elbow = max(right_shoulder_abduction_adduction) - min(right_shoulder_abduction_adduction);

fprintf('Range of Motion per right_shoulder_abduction_adduction: %.2f gradi\n', rom_right_elbow);
...
for window = 1:4
    ...
    angolo_window = angoloSpallaDestroFiltrato(window_start_index:window_end_index);
    
    % Seleziona i dati ottici all'interno della finestra corrente
    opto_window = right_shoulder_abduction_adduction(window_start_index:window_end_index);
    ...
    if window >= 2
        angolo_finestre_selezionate = angoloSpallaDestroFiltrato(window_start_index:window_end_index);
        opto_finestre_selezionate = right_shoulder_abduction_adduction(window_start_index:window_end_index);
...
fprintf('ROM per angoloSpallaDestroFiltrato nelle finestre selezionate:\n');
for window = 1:4
    fprintf('Finestra %d: %.2f gradi\n', window, rom_values(window, 1));
end

fprintf('ROM per right_shoulder_abduction_adduction nelle finestre selezionate:\n');
for window = 1:4
    fprintf('Finestra %d: %.2f gradi\n', window, rom_values(window, 2));
end

% Visualizza le deviazioni standard
fprintf('Deviazione standard per angoloSpallaDestroFiltrato nelle finestre 2, 3 e 4: %.2f gradi\n', mean(dev_std_angolo));
fprintf('Deviazione standard per right_shoulder_abduction_adduction nelle finestre 2, 3 e 4: %.2f gradi\n', mean(dev_std_opto));
