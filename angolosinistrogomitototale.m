...
angoloSinistroGomito = zeros(numFrames, 1);
...
for i = 1:numFrames
    ...
    angoloSinistroGomito(i) = acosd(dotProduct / (normShoulderElbow * normElbowWrist));
end
...
time1 = frames / 60;
figure
plot(time1, angoloSinistroGomito, 'b-', 'LineWidth', 2);

% Personalizza il grafico
title('Angolo sinistro gomito');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
grid on;
...
angoloSinistroGomitoFiltrato = filtfilt(b, a, angoloSinistroGomito);

% Plot dell'angolo destro filtrato rispetto ai frame
figure
plot(time1, angoloSinistroGomitoFiltrato, 'r-', 'LineWidth', 2);

% Personalizza il grafico
title('Angolo sinistro gomito (Filtrato)');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
grid on;
...
frame = xls_data(:, 1);
time2=frame/60;
left_elbow_flexion_extension = xls_data(:, 40);

% Crea il grafico
figure;
plot(time2, left_elbow_flexion_extension, 'b.-');
xlabel('Time (s)');
ylabel('Left Elbow Flexion/Extension');
title('Grafico Left Elbow Flexion/Extension vs. Time');
grid on;
...
if lunghezza_diff < 0
    ...
    left_elbow_flexion_extension = left_elbow_flexion_extension(-lunghezza_diff : end-1);
    time2 = time2(1:length(left_elbow_flexion_extension)); % Aggiorna il secondo vettore tempo
elseif lunghezza_diff > 0
    % Rimuovi gli elementi iniziali in eccesso dal primo angolo
    angoloSinistroGomitoFiltrato = angoloSinistroGomitoFiltrato(lunghezza_diff : end-1);
    time1 = time1(1:length(angoloSinistroGomitoFiltrato)); % Aggiorna il primo vettore tempo
end
...
subplot(2, 1, 1);
plot(time1, angoloSinistroGomitoFiltrato, 'b-', 'LineWidth', 2);
xlabel('Time(s)');
ylabel('Angolo sinistro gomito (gradi)');
title('Angolo sinistro gomito vs. Time');
grid on;

hold on
% Sottoplot per il secondo angolo
%subplot(2, 1, 2);
plot(time2, left_elbow_flexion_extension, 'r-', 'LineWidth', 2);
xlabel('Time(s)');
...
disp('Seleziona un punto su angoloSinistroGomitoFiltrato.');
[x1, ~] = ginput(1);

% Scegli manualmente un punto su right_knee_flexion_extension
disp('Seleziona un punto su left_elbow_flexion_extension.');
[x2, ~] = ginput(1);
...
angoloSinistroGomitoFiltratoSync = interp1(time1_sync, angoloSinistroGomitoFiltrato, time1, 'linear', 'extrap');
angoloSinistroGomitoFiltrato=angoloSinistroGomitoFiltratoSync;
left_elbow_flexion_extension=left_elbow_flexion_extension(1:length(angoloSinistroGomitoFiltrato));
% Aggiorna il grafico sincronizzato sovrapposto
figure
plot(time1, angoloSinistroGomitoFiltrato, 'r-', 'LineWidth', 2);
hold on;
plot(time1, left_elbow_flexion_extension, 'b.-');
hold off;

title('Grafici Sovrapposti - Sincronizzati');
xlabel('Time(s)');
ylabel('Angolo (gradi)');
legend('Angolo sinistro gomito (Filtrato)', 'Left elbow Flexion/Extension');
grid on;
time2=time1;
...
plot(time1, angoloSinistroGomitoFiltrato, 'Color', MATLABcol(1,:), 'LineWidth', 2);
hold on;

% Plot della flessione/estensione continua del ginocchio destro (Xsens)
plot(time2, left_elbow_flexion_extension, 'Color', MATLABcol(2,:), 'LineWidth', 2);

hold off;

title('Left Elbow Flexion/Extension Comparison','interpreter', 'latex');
...
xlim([12, 24]); % Imposta il limite inferiore dell'asse x a 10 secondi
...
[h1, p1, jbstat1, critval1] = jbtest(angoloSinistroGomitoFiltrato);

% Stampare i risultati del test per angoloDestroGinocchioFiltrato
fprintf('Risultati del test di Jarque-Bera per angoloSinistroGomitoFiltrato:\n');
...
[h2, p2, jbstat2, critval2] = jbtest(left_elbow_flexion_extension);

% Stampare i risultati del test per right_knee_flexion_extension
fprintf('\nRisultati del test di Jarque-Bera per left_elbow_flexion_extension:\n');
fprintf('Statistiche JB: %.4f\n', jbstat2);
fprintf('Valore p: %.4f\n', p2);
if h2
    fprintf('I dati NON seguono una distribuzione normale.\n');
...
%Il test di Shapiro-Wilk valuta se un insieme di dati proviene da una distribuzione normale. Si basa sulla differenza tra i valori osservati e quelli attesi in una distribuzione normale, 
%analizzando come questa discrepanza si distribuisce. Il test confronta i valori osservati con quelli attesi se i dati provenissero da una distribuzione normal

%% ICC NUOVO
% Calcola l'ICC
[r, LB, UB, F, df1, df2, p] = ICC([angoloSinistroGomitoFiltrato, left_elbow_flexion_extension], 'A-1', 0.05, 0);

% Stampa i risultati
fprintf('ICC tra angoloSinistroGomitoFiltrato e left_elbow_flexion_extension:\n');
...
% Calcolo del bias
bias = mean(left_elbow_flexion_extension - angoloSinistroGomitoFiltrato);
fprintf('Il bias tra i due segnali è: %.4f\n', bias);

% Calcolo della deviazione standard
std_dev = std(left_elbow_flexion_extension - angoloSinistroGomitoFiltrato);
fprintf('La deviazione standard per la differenza dei  due segnali è: %.4f\n', std_dev);

%CV
% Calcolo del Coefficiente di Variazione (CV)
cv = (std_dev / mean((angoloSinistroGomitoFiltrato + left_elbow_flexion_extension)/2)) * 100;
...
% Bland-Altman plot
BlandAltman(angoloSinistroGomitoFiltrato, left_elbow_flexion_extension, {'Angolo destro ginocchio', 'Angolo ginocchio'});
...
differenza = left_elbow_flexion_extension - angoloSinistroGomitoFiltrato;
media = mean([angoloSinistroGomitoFiltrato, left_elbow_flexion_extension], 2);
...
text(min(media), max(differenza)+10 , sd_label, 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left', 'Color', 'black', 'interpreter', 'latex' );
...
title('Bland-Altman Plot Left Elbow Flexion/Extension', 'interpreter', 'latex');
...
kinect_segment = angoloSinistroGomitoFiltrato(1:i);

opto_segment = left_elbow_flexion_extension(1:i);
...
rom_angoloSinistro = max(angoloSinistroGomitoFiltrato) - min(angoloSinistroGomitoFiltrato);
fprintf('Range of Motion per angoloSinistroGomitoFiltrato: %.2f gradi\n', rom_angoloSinistro);


%% ROM XSENS

% Calcola il ROM per "right_knee_flexion_extension"
rom_left_elbow = max(left_elbow_flexion_extension) - min(left_elbow_flexion_extension);

fprintf('Range of Motion per left_elbow_flexion_extension: %.2f gradi\n', rom_left_elbow);
...
for window = 1:4
    ...
    angolo_window = angoloSinistroGomitoFiltrato(window_start_index:window_end_index);
    
    % Seleziona i dati ottici all'interno della finestra corrente
    opto_window = left_elbow_flexion_extension(window_start_index:window_end_index);
    ...
    if window >= 2
        angolo_finestre_selezionate = angoloSinistroGomitoFiltrato(window_start_index:window_end_index);
        opto_finestre_selezionate = left_elbow_flexion_extension(window_start_index:window_end_index);
...
fprintf('ROM per angoloSinistroGomitoFiltrato nelle finestre selezionate:\n');
for window = 1:4
    fprintf('Finestra %d: %.2f gradi\n', window, rom_values(window, 1));
end

fprintf('ROM per left_elbow_flexion_extension nelle finestre selezionate:\n');
for window = 1:4
...
fprintf('Deviazione standard per angoloSinistroGomitoFiltrato nelle finestre 2, 3 e 4: %.2f gradi\n', mean(dev_std_angolo));
fprintf('Deviazione standard per left_elbow_flexion_extension nelle finestre 2, 3 e 4: %.2f gradi\n', mean(dev_std_opto));
